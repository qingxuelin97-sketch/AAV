...
for (const def of LEVELS) {
  ...
  test(`level ${def.name}: has a goal (flagpole or boss)`, () => {
    if (def.boss) {
      // Boss levels end by defeating Bowser; they intentionally have no flag.
      const hasBoss = def.tiles.some((row) => row.includes(T.BOSS));
      assert.ok(hasBoss, "boss level contains a Bowser spawn");
    } else {
